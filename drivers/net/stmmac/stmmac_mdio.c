 ...
 * stmmac_mdio_dump
 * @bus: points to the mii_bus structure
 * Description: dump the MII registers
 */
void stmmac_mdio_dump(struct mii_bus *bus)
{
	int i;
        struct net_device *ndev = bus->priv;
	struct stmmac_priv *priv = netdev_priv(ndev);

        pr_info("[PHY] Basic register dump :\n");
        for (i = 0; i < 32; i++)
                pr_info("\t%02d %04x\n",i, stmmac_mdio_read(bus, priv->phy_addr, i));
}
...
int stmmac_mdio_reset(struct mii_bus *bus)
{
	...
	if (priv->phy_reset) {
		...
		ret = priv->phy_reset(priv->plat->bsp_priv);
		if (ret) {
			pr_err("[PHY] Reset is failed\n");
			return ret;
		}
	}
...
#ifdef STMMAC_MDIO_DEBUG
	/* dump mii registers */
	stmmac_mdio_dump(bus);
#endif
