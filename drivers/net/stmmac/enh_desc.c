				  ...
				  struct dma_desc *p)
{
	...
	if (unlikely(p->des01.erx.error_summary)) {
		...
		if (unlikely(p->des01.erx.timestamp_csum_giant))
			CHIP_DBG(KERN_ERR "\tTimestamp Available/IPC Csum Error(Type1)/Giant frame\n");
	...
	if(p->des01.erx.extended_status) {
		CHIP_DBG(KERN_ERR "GMAC RX DES4 0x%08x\n", p->des4.erx);
		ret = enh_desc_coe_rdes0(p->des4.erx.ip_header_error,
			p->des01.erx.frame_type, p->des4.erx.ip_payload_error);
	}
