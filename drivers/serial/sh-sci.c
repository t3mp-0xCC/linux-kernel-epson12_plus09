...
static int epson09_console_ctrl_read(
	char *page, char **start, off_t off, int count, int *eof, void *data);
static int epson09_console_ctrl_write(
...
#if defined(CONFIG_ARCH_EPSON09)
	status = sci_in(port, SCxSR);            /* Dummy read */
	sci_out(port, SCxSR, status & SCxSR_TDxE_CLEAR(port));
#else
...
#elif defined(CONFIG_ARCH_EPSON09)
/* EPSON09 doesn't use RTS/CTS */
static void sci_init_pins(struct uart_port *port, unsigned int cflag)
{
...
static void sci_transmit_chars(struct uart_port *port)
{
	...
	/* clear tfr data empty flag cause we refill the tfr data */
#if defined(CONFIG_ARCH_EPSON09)
	status = sci_in(port, SCxSR);
	sci_out(port, SCxSR, status & SCxSR_TDxE_CLEAR(port));
#else
...
static inline void sci_receive_chars(struct uart_port *port)
{
	...
	while (1) {
		if (port->type == PORT_SCI) {
			/*we always reach here for epson09s. */
			count = sci_rxfill(port);
		} else {
			/*we never come here  for epson09s. */
		...
		if (port->type == PORT_SCI) {
			/*we do never reach here for epson09s. */
...
#if defined(CONFIG_ARCH_EPSON09)
		status = sci_in(port, SCxSR); /* dummy read */
		sci_out(port, SCxSR, status & SCxSR_RDxF_CLEAR(port));
#else
...
#if defined(CONFIG_ARCH_EPSON09)
	status = sci_in(port, SCxSR);
	sci_out(port, SCxSR, status & SCxSR_ERROR_CLEAR(port));
#else
	sci_out(port, SCxSR, SCxSR_ERROR_CLEAR(port));
#endif
...
#if defined(CONFIG_ARCH_EPSON09)
	status = sci_in(port, SCxSR);
	sci_out(port, SCxSR, status & SCxSR_BREAK_CLEAR(port)); /* clears Brk, Framing Err, Parity Err. */
#else
...
#if defined(CONFIG_ARCH_EPSON09)
		sci_port->port.uartclk = EPSON09UART_MCLOCK * 1000 * 1000;
#else
...
static void sci_clk_disable(struct uart_port *port)
{
	...
	if (sci_port->fclk != NULL)
		clk_disable(sci_port->fclk);
	if (sci_port->iclk != NULL)
			    ...
			    struct ktermios *old)
{
	...
	if (likely(baud && port->uartclk))
		...
		t = SCBRR_VALUE(115200, EPSON09UART_MCLOCK);
#else
	...
	do {
		status = sci_in(port, SCxSR);
	} while (!(status & SCIF_TEND));

#if defined(CONFIG_ARCH_EPSON09)
	portdata = sci_in(port, SCSPTR);
	sci_out(port, SCSPTR, portdata | 1);
#endif
	...
	if (t > 0) {
#if defined(CONFIG_ARCH_EPSON09)
		{
			...
			sci_out(port, SCSMR, (sci_in(port, SCSMR) & ~3) | (cks & 3));
		}
#else
		if (t >= 256) {
...
static struct uart_driver sci_uart_driver = {
	...
	.dev_name	= "ttyS",
	.major		= SCI_MAJOR,
	.minor		= SCI_MINOR_START + SCI_MINOR_REVISION,
#else
	.dev_name	= "ttySC",
	.major		= SCI_MAJOR,
...
static int __init sci_init(void)
{
	...
	if (entry != NULL) {
		entry->read_proc = epson09_console_ctrl_read;
		entry->write_proc = epson09_console_ctrl_write;
