...
static void ndisc_recv_ns(struct sk_buff *skb)
{
	...
	if (dad) {
#if defined (CONFIG_ARCH_EPSON12)
		if (ipv6_hdr(skb)->saddr.in6_u.u6_addr32[0] == 0 &&
		    ipv6_hdr(skb)->saddr.in6_u.u6_addr32[1] == 0 &&
		    ipv6_hdr(skb)->saddr.in6_u.u6_addr32[2] == 0 &&
		    ipv6_hdr(skb)->saddr.in6_u.u6_addr32[3] == 0) {

			if (ifp->idev->if_flags & IF_RA_RCVD && ifp->idev->if_flags & IF_RS_SENT) {
				ND_PRINTK1("<EP> ndisc_recv_ns::case A\n");
			} else if (ipv6_addr_type(&(msg->target)) & IPV6_ADDR_LINKLOCAL/*0x20*/) {
				ND_PRINTK1("<EP> ndisc_recv_ns::case B\n");
			} else if (!(ifp->idev->if_flags & IF_RA_RCVD) &&
				   !(ifp->idev->if_flags & IF_RS_SENT) &&
				   !(ifp->idev->if_flags & IF_RA_OTHERCONF)){
				ND_PRINTK1("<EP> ndisc_recv_ns::case C\n");
				goto out;
			} else {
				ND_PRINTK1("<EP> ndisc_recv_ns::case D\n");
			}
		}
#endif
